...
class MovieAppTests(TestCase):
    ...
    def setUp(self):
        ...
        self.movie = Movie.objects.create(
            title='Test Movie', genre=self.genre, release_year=2020,
            director='John Doe', description='Test Description',
            added_by=self.user, approved=True  # Updated to be approved by default for list views
        )
    ...
    def test_add_movie_authenticated(self):
        ...
        response = self.client.post(reverse('add_movie'), {
            'title': 'New Movie', 'genre': self.genre.id, 'release_year': 2021,
            'director': 'Jane Doe', 'description': 'New Description'
        })
    ...
    def test_pending_movies_as_staff(self):
        ...
        unapproved_movie = Movie.objects.create(
            title='Pending Movie', genre=self.genre, release_year=2021,
            director='Jane Doe', description='Pending Description',
            added_by=self.user, approved=False
        )
        response = self.client.get(reverse('pending_movies'))
    ...
    def test_pending_articles_as_staff(self):
        ...
        unapproved_article = Article.objects.create(
            title='Pending Article', description='Pending Description',
            added_by=self.user, approved=False
        )
        response = self.client.get(reverse('pending_articles'))
    ...
    def test_average_rating(self):
        # Create reviews
        Review.objects.create(user=self.user, movie=self.movie, rating=5, review='Review 1')
        Review.objects.create(user=self.user, movie=self.movie, rating=7, review='Review 2')
        Review.objects.create(user=self.user, movie=self.movie, rating=9, review='Review 3')
